...
func (uq *UserQuery) QueryPosition() *RoleQuery {
...
// QueryAddby chains the current query on the addby edge.
func (uq *UserQuery) QueryAddby() *BookQuery {
...
func (uq *UserQuery) QueryPreemption() *PreemptionQuery {
	query := &PreemptionQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryRecord() *ResearchQuery {
	query := &ResearchQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithPosition(opts ...func(*RoleQuery)) *UserQuery {
	query := &RoleQuery{config: uq.config}
...
//  WithBooking tells the query-builder to eager-loads the nodes that are connected to
// the "booking" edge. The optional arguments used to configure the query builder of the edge.
...
//  WithAddby tells the query-builder to eager-loads the nodes that are connected to
// the "addby" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithAddby(opts ...func(*BookQuery)) *UserQuery {
	query := &BookQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) WithPreemption(opts ...func(*PreemptionQuery)) *UserQuery {
	query := &PreemptionQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withPreemption = query
	return uq
}
...
func (uq *UserQuery) WithRecord(opts ...func(*ResearchQuery)) *UserQuery {
	query := &ResearchQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withBooking; query != nil {
		...
		for _, n := range neighbors {
			...
			node.Edges.Booking = append(node.Edges.Booking, n)
		}
	}

	if query := uq.withAddby; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.Book(func(s *sql.Selector) {
			s.Where(sql.InValues(user.AddbyColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			node.Edges.Addby = append(node.Edges.Addby, n)
		}
	}
	...
	if query := uq.withPreemption; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.Preemption(func(s *sql.Selector) {
			s.Where(sql.InValues(user.PreemptionColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
	...
	if query := uq.withRecord; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.Research(func(s *sql.Selector) {
			s.Where(sql.InValues(user.RecordColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
