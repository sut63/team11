...
func (aq *AuthorQuery) QueryOwner() *ResearchQuery {
	query := &ResearchQuery{config: aq.config}
...
func (aq *AuthorQuery) QueryWriter() *BookQuery {
...
func (aq *AuthorQuery) FirstXID(ctx context.Context) int {
...
func (aq *AuthorQuery) WithOwner(opts ...func(*ResearchQuery)) *AuthorQuery {
	query := &ResearchQuery{config: aq.config}
...
//  WithWriter tells the query-builder to eager-loads the nodes that are connected to
// the "writer" edge. The optional arguments used to configure the query builder of the edge.
func (aq *AuthorQuery) WithWriter(opts ...func(*BookQuery)) *AuthorQuery {
	query := &BookQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AuthorQuery) sqlAll(ctx context.Context) ([]*Author, error) {
	...
	if query := aq.withOwner; query != nil {
		...
		query.Where(predicate.Research(func(s *sql.Selector) {
			s.Where(sql.InValues(author.OwnerColumn, fks...))
		}))
	...
	if query := aq.withWriter; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Author)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.Book(func(s *sql.Selector) {
			s.Where(sql.InValues(author.WriterColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
