...
// HasCatof applies the HasEdge predicate on the "catof" edge.
func HasCatof() predicate.Category {
	return predicate.Category(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(CatofTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, CatofTable, CatofColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasCatofWith applies the HasEdge predicate on the "catof" edge with a given conditions (other predicates).
func HasCatofWith(preds ...predicate.Book) predicate.Category {
	return predicate.Category(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(CatofInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, CatofTable, CatofColumn),
		)
