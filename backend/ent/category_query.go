...
// QueryCatof chains the current query on the catof edge.
func (cq *CategoryQuery) QueryCatof() *BookQuery {
...
//  WithCatof tells the query-builder to eager-loads the nodes that are connected to
// the "catof" edge. The optional arguments used to configure the query builder of the edge.
func (cq *CategoryQuery) WithCatof(opts ...func(*BookQuery)) *CategoryQuery {
	query := &BookQuery{config: cq.config}
...
func (cq *CategoryQuery) sqlAll(ctx context.Context) ([]*Category, error) {
	...
	if query := cq.withCatof; query != nil {
		...
		query.Where(predicate.Book(func(s *sql.Selector) {
			s.Where(sql.InValues(category.CatofColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.Catof = append(node.Edges.Catof, n)
		}
	}
