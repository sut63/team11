...
func (sq *StatusQuery) QueryStatus() *ClientEntityQuery {
	query := &ClientEntityQuery{config: sq.config}
...
// QueryStatusofbook chains the current query on the statusofbook edge.
func (sq *StatusQuery) QueryStatusofbook() *BookQuery {
	query := &BookQuery{config: sq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *StatusQuery) FirstXID(ctx context.Context) int {
...
func (sq *StatusQuery) WithStatus(opts ...func(*ClientEntityQuery)) *StatusQuery {
	query := &ClientEntityQuery{config: sq.config}
...
//  WithStatusofbook tells the query-builder to eager-loads the nodes that are connected to
// the "statusofbook" edge. The optional arguments used to configure the query builder of the edge.
func (sq *StatusQuery) WithStatusofbook(opts ...func(*BookQuery)) *StatusQuery {
	query := &BookQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withStatusofbook = query
	return sq
}
...
//
//	client.Status.Query().
//		GroupBy(status.FieldSTATUSNAME).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
func (sq *StatusQuery) sqlAll(ctx context.Context) ([]*Status, error) {
	...
	if query := sq.withStatus; query != nil {
		...
		query.withFKs = true
		query.Where(predicate.ClientEntity(func(s *sql.Selector) {
			s.Where(sql.InValues(status.StatusColumn, fks...))
		}))
	...
	if query := sq.withStatusofbook; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Status)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.Book(func(s *sql.Selector) {
			s.Where(sql.InValues(status.StatusofbookColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
